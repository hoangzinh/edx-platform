...
class IndexPageProgramsTests(ModuleStoreTestCase):
    ...
    def test_get_programs_not_called(self):
        with mock.patch("student.views.get_active_programs_list") as patched_get_active_programs:
            # check the /dashboard
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertEqual(patched_get_active_programs.call_count, 0)

        with mock.patch("courseware.views.views.get_active_programs_list") as patched_get_active_programs:
            # check the /courses view
            response = self.client.get(reverse('branding.views.courses'))
            self.assertEqual(response.status_code, 200)
            self.assertEqual(patched_get_active_programs.call_count, 0)
    ...
    def test_get_programs_called(self):
        with mock.patch("student.views.get_active_programs_list") as patched_get_active_programs:
            ...
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertEqual(patched_get_active_programs.call_count, 1)
